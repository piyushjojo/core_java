...
public class StudentCollectionUtils {
	...
	//sort student details (map) as per DoB
	public static List<Student> sortStudentsByDoB(Map<String, Student> students)
	{
		return students.values() //Collection<Student>
				.stream() //Stream<Student>
				.sorted((s1,s2) -> s1.getDob().compareTo(s2.getDob())) //Stream<Student> : sorted as per dob
				.collect(Collectors.toList());
				
	}
	//sort student details (map) as per GPA
		public static List<Student> sortStudentsByGPA(Map<String, Student> students)
		{
			return students.values() //Collection<Student>
					.stream() //Stream<Student>
					.sorted((s1,s2) -> ((Double)s1.getGpa()).compareTo(s2.getGpa())) //Stream<Student> : sorted as per dob
					.collect(Collectors.toList());
